...
class ExcelPriceListParser:
    ...
    def parse(self, sheet_name: str = None) -> Dict:
        ...
        try:
            ...
            if header_row is None:
                logger.warning("Не найдена строка с заголовком 'Наименование' или 'Товар'. Пробуем старый метод.")
                header_row = self._find_header_row(df)
                if header_row is None:
                    header_row = 1  # Fallback: строка 2 (0-based индекс 1) для EuroGips
    ...
    def _find_columns_by_keywords(self, df: pd.DataFrame, header_row: int) -> tuple:
        ...
        for idx, val in enumerate(header):
            ...
            if ('товар' in val_str or 'наименование' in val_str or 'название' in val_str) and product_col is None:
                product_col = idx
            elif ('ед' in val_str or 'единиц' in val_str or 'изм' in val_str) and unit_col is None:
                unit_col = idx
            elif ('цена' in val_str or 'стоимость' in val_str or 'продаж' in val_str) and price_col is None:
                price_col = idx
        
        # Если не нашли, используем стандартные индексы (для EuroGips: Наименование=2, Ед.изм=3, Цена продажи=4)
        if product_col is None:
            # Ищем колонку "Наименование" (обычно колонка 2 для EuroGips)
            for idx, val in enumerate(header):
                val_str = str(val).strip() if pd.notna(val) else ''
                if 'Наименование' in val_str:
                    product_col = idx
                    break
            if product_col is None:
                product_col = 2  # Fallback для EuroGips
        if unit_col is None:
            # Ищем колонку "Ед.изм" (обычно колонка 3 для EuroGips)
            for idx, val in enumerate(header):
                val_str = str(val).strip() if pd.notna(val) else ''
                if 'Ед' in val_str or 'изм' in val_str:
                    unit_col = idx
                    break
            if unit_col is None:
                unit_col = 3  # Fallback для EuroGips
        if price_col is None:
            # Ищем колонку "Цена продажи" (обычно колонка 4 для EuroGips)
            for idx, val in enumerate(header):
                val_str = str(val).strip() if pd.notna(val) else ''
                if 'Цена' in val_str or 'продаж' in val_str:
                    price_col = idx
                    break
            if price_col is None:
                price_col = 4  # Fallback для EuroGips
    ...
    def _parse_data_new(self, df: pd.DataFrame, start_row: int, end_row: int, 
        ...
        """
        Парсинг данных для формата EuroGips:
        - Колонка "Наименование" (product_col) - название товара или категории
        - Колонка "Ед.изм." (unit_col) - единица измерения (только для товаров)
        - Колонка "Цена продажи" (price_col) - цена (только для товаров)
        
        Правила:
        - Если есть "Наименование", но нет "Ед.изм." и "Цена продажи" - это категория
        - Если есть "Наименование", "Ед.изм." и "Цена продажи" - это товар
        """
        ...
        for idx, row in data.iterrows():
            ...
            try:
                if product_col < len(row):
                    product_name = self._to_str_or_none(row.iloc[product_col])
            except (IndexError, KeyError):
                pass
            
            try:
                if unit_col < len(row):
                    unit = self._to_str_or_none(row.iloc[unit_col])
            except (IndexError, KeyError):
                pass
            
            try:
                if price_col < len(row):
                    price = self._to_float_or_nan(row.iloc[price_col])
            except (IndexError, KeyError):
                pass
            
            # Пропускаем полностью пустые строки
            if not product_name and not unit and math.isnan(price):
                continue
            
            # Если есть название, но нет единицы измерения и цены - это категория
            if product_name and (not unit or not self._is_unit_measurement(unit)) and math.isnan(price):
                current_category = product_name
                if current_category not in self.categories:
                    self.categories.append(current_category)
                logger.debug(f"Найдена категория: '{current_category}'")
                continue
            
            # Если есть название, единица измерения и цена - это товар
            if product_name and unit and self._is_unit_measurement(unit) and not math.isnan(price):
                ...
                logger.debug(f"Извлечен товар: {product_name}, цена: {price}, единица: {unit_normalized}, категория: {current_category}")
