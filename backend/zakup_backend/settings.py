...
ALLOWED_HOSTS = os.getenv('ALLOWED_HOSTS', 'localhost,127.0.0.1,zakup.one,www.zakup.one,5.101.67.195').split(',')
...
MIDDLEWARE = [
    ...
    # 'django.middleware.csrf.CsrfViewMiddleware',  # Временно отключен для отладки
...
REST_FRAMEWORK = {
    ...
    'EXCEPTION_HANDLER': 'zakup_backend.exceptions.custom_exception_handler',
}
...
SESSION_COOKIE_SECURE = True  # True для HTTPS (zakup.one использует HTTPS)
...
# Отключаем CSRF для API полностью (используем Token Authentication)
