...
import { Loader2, FileSpreadsheet, Plus, Edit, Save, X, Trash2 } from 'lucide-react'
...
export default function AdminSuppliersManagement() {
  ...
  const handleCreate = async () => {
    try {
      await api.admin.createSupplier({
        name: formData.name,
  ...
  const handleUpdate = async () => {
    if (!editingId) return
    try {
      await api.admin.updateSupplier(editingId, {
        name: formData.name,
        contact_email: formData.contact_email || undefined,
        contact_phone: formData.contact_phone || undefined,
      })
  ...
  const handleToggleActive = async (supplierId: number) => {
    try {
      await api.admin.toggleSupplierActive(supplierId)
      fetchSuppliers()
    } catch (err: any) {
