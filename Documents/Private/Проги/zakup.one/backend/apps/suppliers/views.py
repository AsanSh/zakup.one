...
class PriceListViewSet(viewsets.ModelViewSet):
    ...
    def process(self, request, pk=None):
        ...
        try:
            # TODO: Реализовать парсинг прайс-листа
            price_list.status = 'PROCESSING'
            price_list.save()
            
            # Симуляция обработки
            # В реальности здесь будет парсинг файла
            price_list.status = 'PROCESSED'
            price_list.processed_at = timezone.now()
            price_list.products_count = 0  # TODO: Подсчитать реальное количество
            price_list.save()
                    ...
                    try:
                        ...
                        with transaction.atomic():
                            ...
                            if parsing_method == 'EXCEL':
                                ...
                                logger.info(f'Парсинг завершен. Найдено товаров: {len(result.get("products", []))}, категорий: {len(result.get("categories", []))}')
                                ...
                                pl.log = f"Обработано товаров: {imported_count}, категорий: {len(result.get('categories', []))}"
                                pl.save()
            ...
            try:
                ...
                product, created = Product.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'markup_percent': 0,  # Наценка по умолчанию 0
                        'is_active': True,
                    }
                )
